      ...
      {answer.content.map((item: any, i_index: number) => (
        <div key={i_index}>
          ...
          {item.type == ITEM_TYPE.TEXT && typeof item.data == "string" && (
            <div className=" justify-center flex text-[18px]">
              <Text str={item.data} />
            </div>
          )}
        </div>
...
const SelectImageTA013 = ({ question, is_view }: { question: QuestionRender, is_view ?: boolean }) => {
  return (
...
export default SelectImageTA013;
